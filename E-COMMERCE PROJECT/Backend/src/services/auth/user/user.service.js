...
module.exports = class UserAuthService {
    ...
    async fetchSingleUser(body, isSelect) {
        try {
            if (isSelect) {
                return await User.findOne(body).select('_id first_name last_name email phone gender address profile_image isActive create_at update_at');
            } else {
    ...
    async fetchAllUser() {
        try {
            return await User.find({ isDelete: false }).select('_id first_name last_name email phone gender address profile_image isActive create_at update_at');
        } catch (err) {
